...
"""
    ...
    if not allowed:
        reason = guardrail_reason or (
            "VoyAIge can only help with travel-planning requests. "
...
Format the final answer using markdown headings (##) for each of these sections:
...
Important:
- You MUST use `## ` H2 headings for each section title.
- Be clear, structured, and practical.
- Mention that live flight APIs may not provide ticket prices when pricing is unavailable.
- Include weather-based travel advice.
- Use bullet points and bold text to structure details under each heading cleanly.
